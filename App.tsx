...
import { LinkItem, Category, DEFAULT_CATEGORIES, INITIAL_LINKS, WebDavConfig, AIConfig, SearchMode, ExternalSearchSource, SearchConfig, DisplayTheme } from './types';
...
function App() {
        ...
        if (websiteConfigRes.ok) {
          ...
          if (websiteConfigData) {
            setSiteSettings(prev => {
              ...
              if (!websiteConfigData.faviconApi && authToken) {
                console.log('Detected missing faviconApi in KV, auto-saving...');
                fetch('/api/storage', {
                  method: 'POST',
                  headers: {
                    'Content-Type': 'application/json',
                ...
                }).catch(err => console.error('Auto-save faviconApi failed:', err));
              }
  ...
  // 创建可排序的链接卡片组件
  const SortableLinkCard = ({ link }: { link: LinkItem, [key: string]: any }) => {
    const {
      attributes,
      listeners,
      setNodeRef,
      ...
      >
        ...
        <div className={`flex flex-1 min-w-0 overflow-hidden ${isDetailedView ? 'flex-col' : 'items-center gap-3'
          ...
          <div className={`flex items-center gap-3 mb-2 ${isDetailedView ? '' : 'w-full'
            ...
            {/* 标题 */}
            <h3 className={`${currentTheme.text.title} ${currentTheme.text.darkTitle} truncate overflow-hidden text-ellipsis ${isDetailedView ? 'text-base' : 'text-sm font-medium'
              }`} title={link.title}>
              {link.title}
            </h3>
          </div>
        ...
        {isBatchEditMode ? (
          ...
          >
            ...
            <div className={`flex items-center gap-3 w-full`}>
              ...
              {/* 标题 */}
              <h3 className={`${currentTheme.text.title} ${currentTheme.text.darkTitle} truncate whitespace-nowrap overflow-hidden text-ellipsis ${currentTheme.text.hoverTitle} ${currentTheme.text.darkHoverTitle} transition-colors ${isDetailedView ? 'text-base' : 'text-sm font-medium'
                }`} title={link.title}>
                {link.title}
              </h3>
            </div>
    ...
    <div className="flex h-screen overflow-hidden text-slate-900 dark:text-slate-50">
      ...
      {(!requiresAuth || authToken) && (
        ...
        `}
          ...
          {/* Main Content */}
          <main className={`flex-1 flex flex-col h-full ${currentTheme.mainBg} ${currentTheme.darkMainBg} overflow-hidden relative transition-colors duration-500`}>
            {/* Background Decoration blobs */}
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
              {/* Static Gradient Overlay - 静态渐变层 */}
              <div
                className="absolute inset-0 transition-opacity duration-1000"
                style={{
                  background: siteSettings.displayTheme === 'midnight-galaxy'
                    ? 'linear-gradient(135deg, rgba(126, 34, 206, 0.15) 0%, rgba(30, 64, 175, 0.1) 50%, transparent 100%)'
                    : siteSettings.displayTheme === 'cyberpunk-neon'
                      ? 'linear-gradient(225deg, rgba(147, 51, 234, 0.2) 0%, rgba(8, 145, 178, 0.15) 50%, transparent 100%)'
                      : siteSettings.displayTheme === 'default'
              ...
              <div
                ...
                style={{
                  ...
                  color: siteSettings.displayTheme === 'midnight-galaxy' ? '#7e22ce' : siteSettings.displayTheme === 'cyberpunk-neon' ? '#0891b2' : 'transparent'
                }}
              ></div>

              {/* Pattern Overlay - 增强星空/网格质感 */}
              <div
                className={`absolute inset-0 ${currentTheme.decor.pattern}`}
                style={{
                  backgroundImage: (siteSettings.displayTheme === 'midnight-galaxy' || siteSettings.displayTheme === 'cyberpunk-neon')
                    ? `radial-gradient(circle at 2px 2px, currentColor 1px, transparent 0),
                       radial-gradient(circle at 15px 15px, currentColor 0.5px, transparent 0),
                       radial-gradient(circle at 25px 5px, currentColor 0.5px, transparent 0)`
                    : `radial-gradient(circle at 2px 2px, currentColor 1px, transparent 0)`,
                  backgroundSize: (siteSettings.displayTheme === 'midnight-galaxy' || siteSettings.displayTheme === 'cyberpunk-neon')
                    ? '48px 48px, 64px 64px, 128px 128px'
                    : '32px 32px',
                ...
                <section className="mt-8 pt-8 border-t-2 border-slate-200 dark:border-slate-700">
                  <h2 className="text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 flex items-center gap-2 mb-4">
                    ...
                    <span className="ml-2 px-2 py-0.5 text-xs font-medium bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-300 rounded-full">
                      {(Object.values(otherCategoryResults) as LinkItem[][]).flat().length}
                    </span>
                  </h2>
          ...
          <LinkModal
            ...
            faviconApi={siteSettings.faviconApi}
          />
