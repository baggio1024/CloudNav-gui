...
const LinkModal: React.FC<LinkModalProps> = ({ isOpen, onClose, onSave, onDelete, categories, initialData, aiConfig, defaultCategoryId, faviconApi }) => {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    // 如果有自定义图标URL，缓存到KV空间
    if (icon && !icon.includes(faviconApi)) {
  ...
  const handleFetchIcon = async () => {
    ...
    try {
      ...
      // 如果缓存中没有，则生成新图标
      const iconUrl = `${faviconApi || 'https://favicon.im/'}${domain}?larger=true`;
      setIcon(iconUrl);
